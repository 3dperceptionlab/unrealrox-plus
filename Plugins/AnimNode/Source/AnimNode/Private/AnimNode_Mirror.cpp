...
void FAnimNode_Mirror::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	{
		if (BonesTransfroms.Map_IdxTransform.Contains(b_m.BoneName))
		{
...
void FAnimNode_Mirror::InitializeBoneReferences(const FBoneContainer& RequiredBones)
{
	...
	const TArray< FBoneIndexType > bone_indices = RequiredBones.GetBoneIndicesArray();
	const FReferenceSkeleton skeleton = RequiredBones.GetReferenceSkeleton();
	for (FBoneIndexType indice : bone_indices)
	{
		FName n = skeleton.GetBoneName(indice);
		FBoneReference br(n);
		if (br.Initialize(RequiredBones))
		{
			SetOfBonesToModify.AddUnique(br);
		}
	}
